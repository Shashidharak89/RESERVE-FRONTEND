...
import { X, Copy, Folder, Home } from 'lucide-react';
import { api } from '../../services/api';

export default function CopyModal({ item, onClose, onSubmit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSubmit(item, selectedFolderId);
      onClose();
    } catch (err) {
      ...
      <div className="modal-box">
        <div className="modal-header">
          <div className="modal-title">
            <Copy size={20} />
            <h3>Copy "{item.originalFilename}" to My Vault</h3>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            <label className="section-label">Select Target Private Folder:</label>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn-primary" disabled={submitting}>
              {submitting ? <span className="spinner"></span> : 'Copy Here'}
            </button>
          </div>
        </form>
      </div>
